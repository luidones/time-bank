...
String.prototype.toDate = (function () {
    var knownParts = [
        { pattern: 'dd', prop: 'day' },
        { pattern: 'd', prop: 'day' },
        { pattern: 'mm', prop: 'month' },
        { pattern: 'm', prop: 'month' },
        { pattern: 'yyyy', prop: 'year' },
        { pattern: 'HH', prop: 'hour' },
        { pattern: 'H', prop: 'hour' },
        { pattern: 'MM', prop: 'minute' },
        { pattern: 'M', prop: 'minute' },
        { pattern: 'SS', prop: 'second' },
        { pattern: 'S', prop: 'second' }
    ];
    ...
    return function (format) {
        ...
        if (this) {
            var date = { day: 1, month: 1, year: 1, hour: 0, minute: 0, second: 0 };
            var pattern = format;
            var foundParts = [];

            knownParts.forEach(function (part) {
                if (pattern.indexOf(part.pattern) >= 0) {
                    pattern = pattern.replace(part.pattern, '(#G{' + part.pattern.length + '})');
                    foundParts.push(part);
                }
            });

            for (var i = 0; i < foundParts.length; i++) {
                var part = foundParts[i];
                foundParts[i] = { part: part, index: format.indexOf(part.pattern) };
            }

            foundParts.orderBy(function (part) { return part.index; });

            var regex = new RegExp(pattern.replace(/#G/g, '\\d'));
            ...
            for (var i = 1; i < match.length; i++)
                date[foundParts[i - 1].part.prop] = match[i];

            result = new Date(date.year, date.month - 1, date.day, date.hour, date.minute, date.second, 0);
        }
